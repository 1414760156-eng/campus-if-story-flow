(function attach(root, factory) {
  ...
  function formatMicroGuide(group) {
    ...
    const topic = role.includes("：") ? role.split("：").pop() : role.replace(/^微心态抉择\s*\d*/, "");
    const guides = [
      [/怎么开口/, "母亲在电话那头等一句能落地的话。林亦舟要先决定，是只把行程交出去，还是把家里和留校的压力一起说出来。"],
      [/收住还是补一句|是否补一句/, "第一句已经发出去了，输入框还亮着。再补一句会让关系更清楚，也会让被压住的问题更难装作没看见。"],
      [/先问什么/, "勤工表已经从消息变成入口。他要先问规则、缺口还是代价，决定这张表怎样切进自己的暑假。"],
      [/保哪一种见面/, "见父亲和去光谷之间只隔着一个半小时。任何一边多说几句话，另一边都会被挤掉。"],
      [/怎么把时间落地/, "备忘录里只有两行时间，但这两行会决定谁等、谁改期、谁只能收到一句报平安。"],
      [/先算哪一项/, "车票、饭卡、打印费和可换钱时间都摆在同一页上。他要先把哪一项变成能执行的顺序。"],
      [/先向谁说清/, "三个聊天框都在等具体时间。同一个压力不能复制同一句话发出去。"],
      [/说到什么程度/, "第一句话要让晚风知道光谷会被挤掉。说得越具体，她越不用猜，他也越不能继续含混。"],
      [/预算说到几分/, "只说时间能保住体面，说到钱和勤工就会让压力显形。他要决定说到哪一步。"],
      [/问谁/, "信封、小票和成绩通知压在一起。他要先把钱背后的缺口问向谁。"],
      [/继续追还是收住/, "父亲和母亲的沉默都已经出现。继续追会让缺口变清楚，收住则能先保住当下运转。"],
      [/钱怎么处理/, "这笔钱能让饭卡和打印费稳一点，也会把没说完的话一起塞进包里。"],
      [/先补哪块窟窿/, "父亲给的钱和勤工便签被夹在一起。他要先补眼前哪一个缺口。"],
      [/先补哪份材料/, "C201 门牌、补考资料和勤工申请表都在文件袋里。他要先抽出哪一份。"],
      [/如何承认错过/, "窗口已经被别人接走。接下来不是把错过抹掉，而是决定把它记在哪里、告诉谁。"],
      [/怎么接班/, "临时班第一次有了具体时间。答应得太快会压住复习，问得太慢又可能错过窗口。"],
      [/先说哪件白天事/, "白天被补考、打印、教室和窗口切开。他要先让谁知道自己为什么不能按原时间出现。"],
      [/说到什么深度/, "只报位置能减少解释，说清顺序会增加约束。他要决定别人需要等到什么程度。"],
      [/坐到哪里/, "座位表把他放回别人能看见的位置。他要决定被看见到哪一步。"],
      [/回答到几分/, "饭桌上的关心已经递过来。他可以用一句话挡掉，也可以说出具体卡住的题。"],
      [/离开前说不说/, "后排靠门能让动作更快，也会让 4XX 只能从结果倒推他的去向。"],
      [/怎么把结果带回 4XX/, "窗口已经跑完，回执在手里。现在要决定结果怎么回到公共桌上。"],
      [/如实写到几分/, "去向页面没有给含混留太多空格。写得越清楚，后面的缺席和晚归越不能再退回没想好。"],
      [/空白怎么解释/, "兼职栏可以先空着，但空白会跟着他回到 4XX。他要给这个空白一个能被追问的说法。"],
      [/先问谁/, "联系人不是默认项。父亲要体面，母亲要真实，学校要能找到人。"],
      [/怎么保存顺序/, "号码顺序已经有了答案。保存之前，他还要决定要不要让家里看见这次排序。"],
    ];
