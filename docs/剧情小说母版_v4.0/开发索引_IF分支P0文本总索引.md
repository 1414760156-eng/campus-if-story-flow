...
| P0-D | `R5-STAND` / `R5-ZHOU` / `R5-TANG` / `R5-LUCHEN` / `R5-LIEFLAT` | `开发文本_IF分支P0第五卷站队分歧.md` | 已完成，但保留较多可入池剧情片段 | 可先作为文本池使用；若要拆 JSON，建议再按新格式重拆一版事件表。 |
...
P0-D 目前文本可读性更强，带有较多可入池片段。后续有两种处理方式：

1. 先用现稿直接拆低保版 JSON：共用入口 + 宿舍修补 / 周屿 / 唐骁 / 陆沉 / 摆烂五条路线。
2. 先重拆事件版 P0-D：把每条路线改为“路线定位、入口条件、3-5 个事件、变量、毕业承接”，与 P0-A/B/C/E 统一。

推荐先做第二种，因为 P0-D 是第五卷强冲突核心，后续互动最容易膨胀。
...
1. 先重拆 P0-D 事件版，统一五个 P0 模块的开发格式。
2. 再建立 `IF_route_schema.json` 或等价字段规范，固定路线字段、变量字段和毕业结算字段。
3. 再从 P0-A 至 P0-E 生成第一版 JSON 节点池。
4. 最后决定是否接回正式试玩器或保留为开发数据源。
